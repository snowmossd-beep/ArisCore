...
public class DuelArenaDatabaseManager implements AutoCloseable {
    ...
    private String tableName() {
        return module.getConfig().getString("mysql.table-prefix", "ariscore_")
                + module.getConfig().getString("mysql.table-name-arenas", "duel_arenas");
    }
    ...
    public void init() {
        ...
        if (mysqlEnabled && sqliteEnabled) {
            module.getPlugin().getLogger().warning("[Duel] Cả mysql.enabled và sqlite.enabled đều đang true trong config.yml. " +
                    "Chỉ nên bật 1 trong 2 — sẽ ưu tiên dùng MySQL.");
        } else if (!mysqlEnabled && !sqliteEnabled) {
            module.getPlugin().getLogger().warning("[Duel] Cả mysql.enabled và sqlite.enabled đều đang false trong config.yml. " +
