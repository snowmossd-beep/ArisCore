...
public class TeamWarmupManager {
    ...
    private void sendCountdown(Player player, int seconds) {
        if (module.getPlugin().getConfig().getBoolean("warmup-message.chat", true)) {
            MessageUtil.sendChat(player, "teleporting_chat", s -> s.replace("{seconds}", String.valueOf(seconds)));
        }
        if (module.getPlugin().getConfig().getBoolean("warmup-message.actionbar", true)) {
