...
public class LocationFinder {
    ...
    public static CompletableFuture<Location> findSafe(World world, ConfigurationSection sec, int maxAttempts) {
        ...
        return tryAttempt(world, minX, maxX, minZ, maxZ, minY, maxY, 0, maxAttempts);
    }

    private static CompletableFuture<Location> tryAttempt(
            World world, int minX, int maxX, int minZ, int maxZ,
            int minY, int maxY, int attempt, int maxAttempts) {
        ...
        return world.getChunkAtAsync(x >> 4, z >> 4).thenCompose(chunk -> {
            ...
            if (highestY < minY || highestY > maxY) {
                return tryAttempt(world, minX, maxX, minZ, maxZ, minY, maxY, attempt + 1, maxAttempts);
            }
